...
namespace Game.Core.Shared
{
    ...
    ///   3. Авто-болд ключевых фраз («При разыгрывании», «В конце хода», …).
    ...
    {
        // *N* и *-N* — токен динамического значения. Звёздочки в выводе не показываем.
        ...
        static string SubstituteDynamicValues(string template, IReadOnlyList<int> liveValues)
        {
            ...
            return DynamicToken.Replace(template, m =>
            {
                ...
                if (liveValues != null && slot < liveValues.Count)
                    ...
                    int.TryParse(m.Groups[1].Value, out value);   // фоллбэк = число из текста
                slot++;
                return value.ToString();
            });
        }

        // ── Проход 3: авто-болд ключевых фраз ──
        static string BoldKeywords(string text, string lang)
        {
            ...
            foreach (var phrase in CardTextLocalization.KeywordPhrases(lang))
            {
                ...
                if (idx < 0) continue;
                // Заменяем только первое вхождение фразы (ключевые фразы идут в начале строки эффекта).
