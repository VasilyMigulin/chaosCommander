...
namespace Game.Core.Ecs.Systems
{
    ...
    public sealed class AutoCastSystem : IEcsInitSystem, IEcsRunSystem, IEcsDestroySystem, System.IDisposable
    {
        ...
        public void Run(IEcsSystems systems)
        {
            ...
            // Предыдущий каст ещё крутится — ждём, чтобы касты не наслаивались.
            if (!PipelineGate.IsSettled(world)) return;

            // + отдельно: предыдущий авто-каст мог быть существом, ждущим клетку размещения. Этот тег
            // НЕ входит в PipelineGate.IsSettled (см. её докстринг — RunAiTurnSystem сам его резолвит,
            // унификация с ним дала бы дедлок), поэтому здесь — свой явный чек, как и было исходно.
            if (world.Filter<PendingSelectCellState>().End().GetEntitiesCount() > 0) return;
