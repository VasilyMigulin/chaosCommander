...
namespace Game.Core.Ecs.Systems
{
    ...
    ///
    /// Работает ТОЛЬКО у активного клиента (компонент ставит локальный ввод — RunSelectCellSystem):
    /// каждый шаг/атака публикует штатные CreatureMovedEvent/CreatureAttackedEvent → CollectActionSystem
    /// шлёт ActionMoveData×N (+ActionAttackData) → пассив реплеит их по одному, синк без новых каналов.
    ///
    ...
    ///   • следующая клетка стала ЗАНЯТОЙ (на пути что-то появилось) или скорость кончилась;
    ///   • цель атаки умерла/не смежна к моменту удара — движение уже состоялось, атака просто не бьётся.
    /// AttacksUsed тратится ТОЛЬКО в момент фактической эмиссии атаки (не при клике) — прерванный
    /// маршрут не съедает лимит атак.
        ...
        public void Run(IEcsSystems systems)
        {
            ...
            foreach (var e in _pathFilter.Value)
            {
                ...
                if (path.Steps != null && path.Steps.Count > 0)
                {
                    ...
                    if (_speedPool.Value.Get(e).Remaining <= 0
                ...
                if (_speedPool.Value.Get(e).Remaining <= 0) continue;
